...
print("开始训练！！！")
start = time.time()
devivce = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
#Dataloader中载入数据集、一次处理数量、shuffle代表是否打乱训练顺序
...
loss_list = []
for i in range(40):
    loss_func = nn.CrossEntropyLoss().to(devivce)
